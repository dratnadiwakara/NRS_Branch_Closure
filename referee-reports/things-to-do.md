...
- [ ] Using SOD data, test whether competing banks' branches in the same zip code or county gain deposits in the years following a closure. This directly answers "how are nearby branches affected" and can be estimated as a straightforward diff-in-diff around closure events. Use number of branches closed and open with sophisticated interaction. Align closely with the existing specification.

- [ ] Use branch closures as events and estimate whether county-level or zip-code-level deposit HHI rises in the years following a closure, relative to areas that did not experience closures. This directly quantifies the market structure consequences the referee is asking about.

- [ ] Using CRA data, estimate whether small business lending in a county declines following branch closures, particularly in counties where the closed branch was the primary lender. 

- [ ] Similarly using HMDA data, test whether mortgage origination volumes or approval rates fall in areas that experience net branch reductions, particularly for lower-income borrowers who may rely more heavily on in-person services.

- [ ] After a closure, measure whether the zip code or county falls below some threshold of branch density (e.g., zero branches remaining, or below one branch per X residents). Document what fraction of closures create banking deserts and whether this is concentrated in particular demographic groups — this directly speaks to "who is hurt."

- [ ] Test whether closures by one bank in a zip code predict closures by other banks in the same area within the following one to two years. This is a simple regression of competitor closure rates on own-bank closure indicators, and speaks directly to whether de-branching is a coordinated or cascading phenomenon.
---
