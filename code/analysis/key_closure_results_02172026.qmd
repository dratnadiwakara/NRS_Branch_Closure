...
```
```{r}
base_name <- sub("\\.rds$", "", prepared_path)
num_parts <- 6

file_info <- file.info(prepared_path)
file_size <- file_info$size
chunk_size <- ceiling(file_size / num_parts)

# --- PART 1: SPLIT ---
# Open the original file in Read Binary mode
con_in <- file(prepared_path, "rb")

part_files <- character(num_parts)
for (i in 1:num_parts) {
  part_files[i] <- paste0(base_name, "_part", i, ".rds")
  
  # Read the specific chunk of bytes
  chunk_data <- readBin(con_in, what = "raw", n = chunk_size)
  
  # Write the chunk to a new part file
  con_out <- file(part_files[i], "wb")
  writeBin(chunk_data, con_out)
  close(con_out)
  
  message("Created: ", part_files[i])
}
close(con_in)
...
setFixest_fml(
  ...
  ..ctrl_county_closure = ~ log(lag_branch_deposit_amount) + same_zip_prior_3yr_branches + 
                           ...
                           log1p(family_income)| county_yr + bank_yr
)
