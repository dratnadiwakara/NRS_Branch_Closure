...
dt_county[, low_income_county := fifelse(median_income_county_yr < median(median_income_county_yr, na.rm = TRUE), 1L, 0L),by=year]
...
setFixest_fml(
  ..fe       = ~ county_code + year,
  ..controls = ~ log1p(branches_county_lag1)+county_growth_3yr+log1p(total_deps_county_lag1),
...
dt_full <- dt_county[year>=2004 & ! year %in% c(2008:2011,2020:2021)]
...
```{r}
# Does the "new branches hurt mortgages in LMI" effect only appear post-crisis?
r_mtg_pre  <- feols(gr_mortgage_1yr ~ (fraction_of_branches_closed + 
                    fraction_of_new_branches) * lmi_high + ..controls | ..fe,
                    data = dt_county[year %in% 2000:2007], vcov = ~ county_code)

r_mtg_post <- feols(gr_mortgage_1yr ~ (fraction_of_branches_closed + 
                    fraction_of_new_branches) * lmi_high + ..controls | ..fe,
                    data = dt_county[year %in% 2012:2019], vcov = ~ county_code)

etable(r_mtg_pre, r_mtg_post)
```
...
```{r}
# =============================================================================
# PART 5b: Same heterogeneity table — year <= 2012
# =============================================================================

dt_pre2012 <- dt_county[year %in% SAMPLE_YEARS & year <= 2012]
r_het_pre <- list()
r_het_pre[["Mtg (baseline)"]] <- feols(
  gr_mortgage_1yr ~ fraction_of_branches_closed + fraction_of_new_branches + ..controls | ..fe,
  data = dt_pre2012, vcov = ~ county_code
)
r_het_pre[["Mtg × low income"]] <- feols(
  gr_mortgage_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * low_income_county +
    ..controls | ..fe,
  data = dt_pre2012, vcov = ~ county_code
)
if ("lmi_high" %in% names(dt_county)) {
  r_het_pre[["Mtg × LMI"]] <- feols(
    gr_mortgage_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * lmi_high +
      ..controls | ..fe,
    data = dt_pre2012, vcov = ~ county_code
  )
}
r_het_pre[["CRA (baseline)"]] <- feols(
  gr_cra_1yr ~ fraction_of_branches_closed + fraction_of_new_branches + ..controls | ..fe,
  data = dt_pre2012, vcov = ~ county_code
)
r_het_pre[["CRA × low income"]] <- feols(
  gr_cra_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * low_income_county +
    ..controls | ..fe,
  data = dt_pre2012, vcov = ~ county_code
)
if ("lmi_high" %in% names(dt_county)) {
  r_het_pre[["CRA × LMI"]] <- feols(
    gr_cra_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * lmi_high +
      ..controls | ..fe,
    data = dt_pre2012, vcov = ~ county_code
  )
}
...
```{r}
# =============================================================================
# PART 5c: Same heterogeneity table — year > 2012
# =============================================================================

dt_post2012 <- dt_county[year %in% SAMPLE_YEARS & year > 2012]
r_het_post <- list()
r_het_post[["Mtg (baseline)"]] <- feols(
  gr_mortgage_1yr ~ fraction_of_branches_closed + fraction_of_new_branches + ..controls | ..fe,
  data = dt_post2012, vcov = ~ county_code
)
r_het_post[["Mtg × low income"]] <- feols(
  gr_mortgage_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * low_income_county +
    ..controls | ..fe,
  data = dt_post2012, vcov = ~ county_code
)
if ("lmi_high" %in% names(dt_county)) {
  r_het_post[["Mtg × LMI"]] <- feols(
    gr_mortgage_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * lmi_high +
      ..controls | ..fe,
    data = dt_post2012, vcov = ~ county_code
  )
}
r_het_post[["CRA (baseline)"]] <- feols(
  gr_cra_1yr ~ fraction_of_branches_closed + fraction_of_new_branches + ..controls | ..fe,
  data = dt_post2012, vcov = ~ county_code
)
r_het_post[["CRA × low income"]] <- feols(
  gr_cra_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * low_income_county +
    ..controls | ..fe,
  data = dt_post2012, vcov = ~ county_code
)
if ("lmi_high" %in% names(dt_county)) {
  r_het_post[["CRA × LMI"]] <- feols(
    gr_cra_1yr ~ (fraction_of_branches_closed + fraction_of_new_branches) * lmi_high +
      ..controls | ..fe,
    data = dt_post2012, vcov = ~ county_code
  )
