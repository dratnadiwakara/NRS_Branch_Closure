...
```
```{r}
dt_combined <- merge(dt_combined,zip_county,by.x="ZIPBR",by.y="ZIP",all.x=T)
...
setFixest_fml(
  ...
  ..controls = ~  log1p(branches_zip_lag1)+ log1p(n_incumbent_banks),#+ log1p(total_deps_zip_lag1) + log1p(n_incumbent_banks),zip_growth_3yr
...
r[["2000-7 (1)"]] <- feols(
  gr_1yr ~ fraction_of_branches_closed + fraction_of_new_branches + ..controls | ..fe_cty_yr,
